...
public class Interpreter {
	...
	private class StmtInterpreter implements ImcVisitor<MemLabel, Object> {
		...
		private void call(ImcCALL imcCall) {
			...
			if (imcCall.label.name.equals("_getInt")) {
				...
				memST(tempLD(SP), Long.valueOf(s.toString().trim()), false);
				return;
			}

			if (imcCall.label.name.equals("_randInt")) {
				Long min = memLD(tempLD(SP, false) + 1 * 8, false);
				Long max = memLD(tempLD(SP, false) + 2 * 8, false);
				memST(tempLD(SP), min + (long)(Math.random() * ((max - min) + 1)), false);
				return;
			}
